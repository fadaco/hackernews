...
export default function IdentifyAsScreen({ route, navigation }: any) {
  const { profile } = route.params;
  const { identify_as } = useSelector((state: any) => state.onboarding);
  const [loading, setLoading] = useState<boolean>(false);
  const [categories, setCategories] = useState<string[]>(IDENTIFY)
  const [value, setValue] = useState<string>(identify_as || '')
